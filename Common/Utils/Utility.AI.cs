...
public static partial class Utility
{
    ...
    public static class AStar
    {
        private class BinaryHeap<T> where T : IComparable<T>
        {
            ...
            {
                var index = 0;
                while (index < Values.Count && Values[index].CompareTo(value) < 0)
                {
                    index++;
                }
        ...
        public record Node(Point Position, int GCost, int HCost, Node Previous) : IComparable<Node>
        {
        ...
        public static IEnumerable<Vector2> FindPath(Rectangle hitbox, Vector2 target, int resolution = 16, int maxIterations = 1000)
        {
            var targetScaled = new Point((int)target.X / resolution, (int)target.Y / resolution);

            var open = new BinaryHeap<Node>();

            var startScaled = new Point(hitbox.Center.X / resolution, hitbox.Center.Y / resolution);
            var closest = new Node(startScaled, 0, Distance(startScaled, targetScaled), null);
            open.Push(closest);

            var closed = new HashSet<Point>();
            for (int iteration = 0; iteration < maxIterations && open.Values.Count > 0; iteration++)
            {
                var current = open.Pop();
                if (current.Position == targetScaled)
                {
                    var shortest = new List<Vector2>();
                    while (current.Previous is not null)
                    {
                        shortest.Add(current.Position.ToVector2() * resolution);
                ...
                for (var i = -1; i <= 1; i++)
                {
                    for (var j = -1; j <= 1; j++)
                    {
                        ...
                        var position = new Point(current.Position.X + i, current.Position.Y + j);
                        if (closed.Contains(position) ||
                            Collision.SolidCollision(position.ToVector2() * resolution + 0.5f * resolution * Vector2.One - 0.5f * new Vector2(hitbox.Width, hitbox.Height),
                                hitbox.Width,
                                hitbox.Height))
                        {
                            continue;
                        ...
                        var newNode = new Node(position, current.GCost + Distance(position, current.Position), Distance(position, targetScaled), current);
                        var oldNode = open.Values.FirstOrDefault(node => node.Position == newNode.Position);
                        if (oldNode is null || newNode.FCost < oldNode.FCost)
                        {
                            open.Push(newNode);
                        }
                    }
            ...
            while (closest.Previous is not null)
            {
                path.Add(closest.Position.ToVector2() * resolution);
